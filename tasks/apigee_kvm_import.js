...
module.exports = function(grunt) {
  ...
  // creation: http://gruntjs.com/creating-tasks
   ...
   grunt.registerMultiTask('apigee_kvm_import', 'Grunt plugin to import KVMs.', function() {
  ...
  function upsertKVM(filepath, cb2){
    if(filepath.options.type === 'env'){
      apigeeSdk.getKVMsEnvironment(grunt.config.get("apigee_profiles")[grunt.option('env')],
      ...
      apigeeSdk.getKVMsOrganization(grunt.config.get("apigee_profiles")[grunt.option('env')],/*{'test' : {url_mgmt : 'https://api.enterprise.apigee.com', org : grunt.option('org'), env : grunt.option('env'), username : grunt.option('username'), password : grunt.option('password')}, env : grunt.option('env')},*/
        function(error, response, body){
          grunt.log.debug(response.statusCode);grunt.log.debug(body);
          upsertKVMList(body, filepath, cb2);
        }, grunt.option('curl'));
